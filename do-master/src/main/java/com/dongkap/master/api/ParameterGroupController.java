...
public class ParameterGroupController extends BaseControllerException {
	...
	@RequestMapping(value = "/vw/auth/parameter-group/v.1/{param}", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<ParameterGroupDto> getParameterGroup(Authentication authentication,
			@PathVariable(required = true) String param) throws Exception {
		return new ResponseEntity<ParameterGroupDto>(this.parameterGroupService.getParameterGroup(param), HttpStatus.OK);
	}
